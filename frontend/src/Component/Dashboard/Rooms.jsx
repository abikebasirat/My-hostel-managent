...
const Room = () => {
    ...
    useEffect(() => {
        const filteredRooms = roomData.filter((res) => {
         ...
         const roomStatus = res.roomStatus?.toLowerCase() || "";
            ...
            return (
                roomLocation.includes(search.toLocaleLowerCase()) ||
                roomStatus.includes(search.toLocaleLowerCase()) 
            );
        });
    ...
    const handleUpdateRoom = (updatedRoomData) => {
       setRoomData((prevData) => prevData.map((room) => room._id === updatedRoomData._id ? 
